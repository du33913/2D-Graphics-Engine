...
private:
  ...
  GShader *shade1;
public:
  DoubleShader(GShader* shader0, GShader* shader1): shade0(shader0), shade1(shader1) {}
  ...
  bool isOpaque() override {
   ...
   void shadeRow(int x, int y, int count, GPixel row[]) override {
    std::vector<GPixel> holder0;
    holder0.resize(count + 1); 
    GPixel* row0 = holder0.data();

    std::vector<GPixel> holder1;
    holder1.resize(count + 1); 
    GPixel* row1 = holder1.data();
    // GPoint points[count];
    //    GPixel row0[count+1];
    //    GPixel row1[count+1];
